...
import (
	...
	"github.com/DataDog/datadog-agent/pkg/clusteragent/admission/validate/alwaysadmit"
...
type Webhook interface {
	...
	WebhookFunc() func(request *admission.Request) *admiv1.AdmissionResponse
}
...
// the agent sidecar webhook needs to go after the config one.
// The reason is that the volume mount for the APM socket added by the config webhook
...
func generateWebhooks(wmeta workloadmeta.Component, pa workload.PodPatcher) []Webhook {
	...
	// APM Instrumentation webhook needs to be registered after the config webhook.
