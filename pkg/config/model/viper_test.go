...
func TestLoadEnvAsStringSlice(t *testing.T) {
	config := NewConfig("test", "DD", strings.NewReplacer(".", "_"))

	config.BindEnv("slice_of_string")
	config.LoadEnvAsStringSlice("slice_of_string", func(string) []string { return []string{"a", "b", "c"} })

	t.Setenv("DD_SLICE_OF_STRING", "__some_data__")
	assert.Equal(t, []string{"a", "b", "c"}, config.Get("slice_of_string"))
}

func TestLoadEnvAsMapStringInterface(t *testing.T) {
	config := NewConfig("test", "DD", strings.NewReplacer(".", "_"))

	config.BindEnv("map_of_float")
	config.LoadEnvAsMapStringInterface("map_of_float", func(string) map[string]interface{} { return map[string]interface{}{"a": 1.0, "b": 2.0, "c": 3.0} })
...
func TestLoadEnvAsSliceMapString(t *testing.T) {
	config := NewConfig("test", "DD", strings.NewReplacer(".", "_"))

	config.BindEnv("map")
	config.LoadEnvAsSliceMapString("map", func(string) []map[string]string { return []map[string]string{{"a": "a", "b": "b", "c": "c"}} })
