...
//
// Those two struct are used to pull data from the configuration into typed struct. We currently load the data from the
// configurarion into MappingProfileConfig and then convert it to MappingProfile.
//
// Both MappingProfileConfig and MetricMappingConfig will be removed once we remove `UnmarshalKey` method from the
// configurarion.
//
