...
func entityIDsFromAttributes(attrs pcommon.Map) []types.EntityID {
	...
	if containerID, ok := attrs.Get(conventions.AttributeContainerID); ok {
		entityIDs = append(entityIDs, types.NewEntityID(common.ContainerID, containerID.AsString()))
	}
	if containerImageID, ok := attrs.Get(conventions.AttributeContainerImageID); ok {
		...
		if len(splitImageID) == 2 {
			entityIDs = append(entityIDs, types.NewEntityID(common.ContainerImageMetadata, splitImageID[1]))
		}
	}
	if ecsTaskArn, ok := attrs.Get(conventions.AttributeAWSECSTaskARN); ok {
		entityIDs = append(entityIDs, types.NewEntityID(common.ECSTask, ecsTaskArn.AsString()))
	}
	if deploymentName, ok := attrs.Get(conventions.AttributeK8SDeploymentName); ok {
		...
		if namespaceOk {
			entityIDs = append(entityIDs, types.NewEntityID(common.KubernetesDeployment, fmt.Sprintf("%s/%s", namespace.AsString(), deploymentName.AsString())))
		}
	}
	if namespace, ok := attrs.Get(conventions.AttributeK8SNamespaceName); ok {
		entityIDs = append(entityIDs, types.NewEntityID(common.KubernetesMetadata, string(util.GenerateKubeMetadataEntityID("", "namespaces", "", namespace.AsString()))))
	}

	if nodeName, ok := attrs.Get(conventions.AttributeK8SNodeName); ok {
		entityIDs = append(entityIDs, types.NewEntityID(common.KubernetesMetadata, string(util.GenerateKubeMetadataEntityID("", "nodes", "", nodeName.AsString()))))
	}
	if podUID, ok := attrs.Get(conventions.AttributeK8SPodUID); ok {
		entityIDs = append(entityIDs, types.NewEntityID(common.KubernetesPodUID, podUID.AsString()))
	}
	if processPid, ok := attrs.Get(conventions.AttributeProcessPID); ok {
		entityIDs = append(entityIDs, types.NewEntityID(common.Process, processPid.AsString()))
	}
	return entityIDs
}
